...
const Command: FunctionComponent<CommandProps> = ({
  ...
  switch (command.command) {
    ...
    case "help":
      content = (
        <p>
          Type `ls` to see the document structure, `cd folder` to navigate into
          a folder, `cd ..` to navigate back and `clear` to clear the console.
        </p>
      );
      break;
    default:
  ...
  return (
    <>
      <PathWrapper path={prevPath}>{command.command}</PathWrapper>
      {content}
      {error}
    </>
  );
