...
const App = () => {
  ...
  const handleKeydown = useCallback(
    (event: KeyboardEvent) => {
      ...
      const {
        ...
        path,
        append,
      } = getCommandsAndPath({ path: pathRef.current, command: text, data });
  ...
  const lastPath = commands.length
    ? commands[commands.length - 1].commandPath
    : "root";
  ...
  return (
    <div className="app">
      ...
      />
      {displayContent &&
        commands.map((commandData, index) => {
          const prevPath = index > 0 ? commands[index - 1].commandPath : "root";
          return (
            <Command
              command={commandData}
              data={data}
              key={commandData.commandPath + index}
              prevPath={prevPath}
            />
          );
        })}
